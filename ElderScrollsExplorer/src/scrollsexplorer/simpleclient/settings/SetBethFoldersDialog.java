...
public class SetBethFoldersDialog extends JDialog {
	...
	private void setFolder(GameConfig gameConfig, JTextField output) {
		...
		while (confirmedFolder == null) {
			File pickedfolder = TitledJFileChooser.requestFolderName("Select " + gameConfig.gameName + " data folder",
					startFolder, SetBethFoldersDialog.this);
			...
			if (!checkEsm.exists()) {
				int r = JOptionPane.showConfirmDialog(
						...
						"The selected folder does not contain " + gameConfig.mainESMFile
													+ " this game type will not work.\n" + "Do you wish to set anyway?",
						"ESM file not found", JOptionPane.YES_NO_OPTION);
			...
			if (countOfBsa == 0) {
				int r = JOptionPane.showConfirmDialog(SetBethFoldersDialog.this,
						"The selected folder does not contain " + " any .bsa files, this game type will not work.\n"
																					+ "Do you wish to set anyway?",
						"No BSA files found", JOptionPane.YES_NO_OPTION);
